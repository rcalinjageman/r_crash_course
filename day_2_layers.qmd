...
#| label: setup
#| message: false
library(tidyverse)    # This loads ggplot2 and a bunch of related packages

library(ggplot2)      # This loads ggplot2 only
```
...
In @sec-day-1-first-graph you learned about faceting with `facet_wrap()`, which splits a plot into subplots that each display one subset of the data based on a categorical variable.
...
The algorithm used to calculate new values for a graph is called a **stat**, short for statistical transformation. @fig-vis-stat-bar shows how this process works with `geom_bar()`.
