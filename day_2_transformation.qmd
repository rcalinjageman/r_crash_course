# Data transformation {#sec-day-2-data-transform}
...
The We'll come back to these comparisons and logical operators in more detail in the R for Data Science book (<https://r4ds.hadley.nz/>).
...
If you want to find the number of occurrences instead, you're better off swapping `distinct()` for `count()`. With the `sort = TRUE` argument, you can arrange them in descending order of the number of occurrences. You can learn more about count in the R for Data Science book (<https://r4ds.hadley.nz/>)
...
By default, `mutate()` adds new columns on the right-hand side of your dataset, which makes it difficult to see what's happening here. We can use the `.before` argument to instead add the variables to the left-hand side[^day_2_transformation-2]:

[^day_2_transformation-2]: Remember that in RStudio, the easiest way to see a dataset with many columns is `View()`.
...
See `?select` for more details. Once you know regular expressions (see the in the R for Data Science book, <https://r4ds.hadley.nz/>) you'll also be able to use `matches()` to select variables that match a pattern.
...
To add the pipe to your code, we recommend using the built-in keyboard shortcut Ctrl/Cmd + Shift + M. You'll need to make one change to your RStudio options to use `|>` instead of `%>%` as shown in @fig-pipe-options; more on `%>%` shortly.
...
Uh-oh! Something has gone wrong, and all of our results are `NA`s (pronounced "N-A"), R's symbol for missing value. This happened because some of the observed flights had missing data in the delay column, and so when we calculated the mean including those values, we got an `NA` result. You can find out more about missing values in detail in the R for Data Science book (<https://r4ds.hadley.nz/>), but for now, we'll tell the `mean()` function to ignore all missing values by setting the argument `na.rm` to `TRUE`:
...
We didn't focus on this syntax in this chapter because it was very new when we wrote the book. We did want to mention it because we think it has a lot of promise and it's likely to be quite popular. You can learn more about it in the [dplyr 1.1.0 blog post](https://www.tidyverse.org/blog/2023/02/dplyr-1-1-0-per-operation-grouping/).
...
1.  The variation in `performance` is larger among players with fewer at-bats. The shape of this plot is very characteristic: whenever you plot a mean (or other summary statistics) vs. group size, you'll see that the variation decreases as the sample size increases[^day_2_transformation-4].
...
[^day_2_transformation-4]: \*cough\* the law of large numbers \*cough\*.
